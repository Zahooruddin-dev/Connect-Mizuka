...
async function createChannel(req, res) {
  ...
  if (!name || !institute_id) {
    return res.status(400).json({ message: 'Missing channel name or institute ID' });
  }

  try {
    const isAuthorized = await dbInstitute.verifyAdminOfInstitute(adminId, institute_id);
    
    if (!isAuthorized) {
      return res.status(403).json({ message: 'Access Denied: You do not manage this institute' });
    }
...
async function updateChannel(req, res) {
  ...
  try {
    ...
    const isAuthorized = await dbInstitute.verifyAdminOfInstitute(adminId, existing.institute_id);
    if (!isAuthorized) {
      return res.status(403).json({ message: 'Access Denied: Institute management required' });
    }
...
async function deleteChannelById(req, res) {
  ...
  try {
    ...
    const isAuthorized = await dbInstitute.verifyAdminOfInstitute(adminId, channel.institute_id);
    if (!isAuthorized) {
      return res.status(403).json({ message: 'Access Denied: Institute management required' });
    }
