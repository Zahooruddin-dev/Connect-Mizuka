...
module.exports = (io) => {
    io.on('connection', (socket) => {
        ...
        socket.on('delete_p2p_message', async (data) => {
            ...
            const members = await require('../db/queryP2P').getChatroomMembers(data.roomId);
        ...
        socket.on('call:user', ({ toUserId, offer, callType, callerUsername, callerId }) => {
        ...
        socket.on('call:accepted', ({ to, answer, callType }) => {
            socket.to(to).emit('call:answered', { from: socket.id, answer, callType });
        });
