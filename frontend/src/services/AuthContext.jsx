...
export function AuthProvider({ children }) {
  ...
  function login(userData, tokenValue) {
    ...
    if (userData.institute_id) {
      const existing = JSON.parse(localStorage.getItem('mizuka_institutes') || '[]')
      const alreadyHas = existing.find(i => i.id === userData.institute_id)
      if (!alreadyHas) {
        const updated = [...existing, { id: userData.institute_id, label: userData.institute_id }]
        setInstitutes(updated)
        localStorage.setItem('mizuka_institutes', JSON.stringify(updated))
        const active = { id: userData.institute_id, label: userData.institute_id }
        ...
        if (!activeInstitute) {
          ...
          localStorage.setItem('mizuka_active_institute', JSON.stringify(existing[0]))
        }
      }
    }
  }
