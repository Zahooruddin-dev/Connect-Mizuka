...
const inputCls =
	'w-full px-3.5 py-2.5 bg-[var(--bg-input)] border border-[var(--border)] rounded-[var(--radius-md)] text-[var(--text-primary)] text-sm outline-none transition-[border-color,box-shadow] duration-200 placeholder:text-[var(--text-ghost)] focus:border-teal-500 focus:ring-2 focus:ring-teal-500/10 mb-4';
const btnCls =
	'w-full px-5 py-3 bg-teal-600 hover:bg-teal-500 text-[var(--text-primary)] text-sm font-medium rounded-[var(--radius-md)] transition-[background-color,opacity] duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-teal-700 focus-visible:outline-offset-2';
...
export default function InstituteGate() {
	...
	return (
		<div className='min-h-svh w-full flex items-center justify-center bg-[var(--bg-base)] relative overflow-hidden px-4'>
			...
			<div className='relative w-full max-w-[440px] px-6 py-10 sm:px-10 bg-[var(--bg-surface)] border border-[var(--border)] rounded-[var(--radius-xl)] shadow-md flex flex-col animate-[card-enter_0.4s_cubic-bezier(0.16,1,0.3,1)_both]'>
				...
				<div className='text-center mb-6'>
					...
					<p className='text-[13px] text-[var(--text-muted)] leading-relaxed'>
						You're not part of any institute yet. Enter an Institute ID to join
						one and start chatting.
					</p>
				</div>
					...
					<div className='mt-6 pt-6 border-t border-[var(--border)]'>
						...
						<form onSubmit={handleCreate} noValidate>
							...
							<input
								...
								autoComplete='off'
							/>
							<button className={btnCls} type='submit' disabled={createLoading}>
