...
export function useCallManager({ user, onToast }) {
  ...
  const acceptCall = useCallback(async () => {
    ...
    try {
      const { callType, callerSocketId, offer, callerUsername, callerUserId } = cs;
      const stream = await getMedia(callType);
      localStreamRef.current = stream;
      remoteSocketIdRef.current = callerSocketId;
      remoteUserIdRef.current = callerUserId; // store remote user ID for later
      const pc = buildPC(stream);
  ...
  const rejectCall = useCallback(() => {
    ...
    socket.emit('call:rejected', {
      to: cs.callerSocketId,
      toUserId: cs.callerUserId,
    });
    setCallState(null);
  }, []);
  ...
  const endCall = useCallback(() => {
    ...
    if (toUserId) {
      ...
      } else if (cs.phase === 'outgoing') {
        socket.emit('call:cancel', { toUserId });
      } else if (cs.phase === 'active') {
  ...
  useEffect(() => {
    const onIncoming = ({
      ...
      if (callStateRef.current) {
        socket.emit('call:rejected', { to: from, toUserId: fromUserId });
        return;
      }
      setCallState({
        phase: 'incoming',
