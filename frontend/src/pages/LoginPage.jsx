...
function inputCls(invalid) {
	return `w-full px-3.5 py-2.5 border rounded-lg text-sm outline-none transition-all duration-150 appearance-none font-[inherit] placeholder:text-[var(--text-ghost)] bg-[var(--bg-input)] text-[var(--text-primary)] focus:ring-2 focus:ring-offset-0 ${
		invalid
			? 'border-red-400 focus:border-red-400 focus:ring-red-400/20'
			: 'border-[var(--border)] focus:border-teal-500 focus:ring-teal-500/20'
	}`;
}

const labelCls =
	'block text-[11px] font-medium uppercase tracking-wide text-[var(--text-secondary)] mb-1.5 mt-3.5 first:mt-0';

const btnCls =
	'w-full mt-5 px-5 py-3 bg-teal-600 hover:bg-teal-500 text-white text-sm font-medium rounded-lg transition-colors duration-150 disabled:opacity-40 disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-teal-700';
	...
	return (
      ...
      `}</style>
			...
			<div className='min-h-svh w-full flex items-center justify-center bg-[var(--bg-base)] relative overflow-hidden px-4'>
				...
				<div className='relative w-full max-w-[420px] px-6 py-8 sm:px-8 bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl shadow-lg flex flex-col animate-[card-enter_0.4s_cubic-bezier(0.16,1,0.3,1)_both]'>
					{/* Logo */}
					<div
						className='flex items-baseline gap-0.5 mb-6'
						aria-label='Mizuka Connect'
					>
						<span
						...
						<span
							className='text-[26px] font-light text-[var(--text-primary)] tracking-[-0.5px] ml-1'
							aria-hidden='true'
						>
							Connect
						</span>
					...
					{info && (
						<p
							role='status'
							aria-live='polite'
							className='text-[13px] text-teal-400 bg-teal-500/10 border border-teal-500/20 rounded-lg px-3 py-2.5 mb-4'
						>
							{info}
						</p>
					)}
					{error && (
						<p
							role='alert'
							aria-live='assertive'
							className='text-[13px] text-red-400 bg-red-400/10 border border-red-400/20 rounded-lg px-3 py-2.5 mb-4'
						>
							{error}
						</p>
					)}
					...
					{view === VIEWS.LOGIN && (
						<form onSubmit={handleLogin} noValidate aria-label='Sign in form'>
							...
							<button
								...
								type='submit'
								disabled={
									loading || (touched.email && !isValidEmail(form.email))
								}
								aria-busy={loading}
							>
							...
							<div className='flex justify-center gap-5 mt-4'>
								<button
									type='button'
									className='text-[13px] text-[var(--text-muted)] hover:text-teal-400 transition-colors duration-150 focus-visible:outline-2 focus-visible:outline-teal-700 rounded-sm'
					...
					{view === VIEWS.RESET_CONFIRM && (
						...
						>
							...
							<button
								...
								disabled={
									loading ||
									(touched.newPassword && form.newPassword.length < 6)
								}
								aria-busy={loading}
							>
